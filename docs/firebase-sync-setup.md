...
## ホーム画面に追加したwebアプリでのログイン

ホーム画面から起動したwebアプリ（standalone表示）では、ポップアップ方式のGoogleログインが完了しません。ポップアップが別のブラウザとして開き、元の画面へ結果を返せないためです。この状態では「Googleログインを確認しています…」の表示から進みません。

`news-sync.js` はstandalone表示を検出し、ページごとGoogleのログイン画面へ移動する方式へ自動で切り替えます。ポップアップが開けない設定のブラウザや、25秒待っても応答がない場合も同じ方式に切り替わります。Firebase側の追加設定は不要です（認証ハンドラーのドメインはポップアップ方式と同じです）。

ページ移動方式は、サイトのドメイン（`taku3516.github.io`）と認証ハンドラーのドメイン（`shinagawakugiakidb.firebaseapp.com`）が異なるため、ブラウザのトラッキング防止機能の影響を受ける場合があります。ログイン後に「ログインを完了できませんでした」と表示される場合は、ブラウザで直接サイトを開いてログインしてください。恒久的に解消するには、認証ハンドラーをサイトと同じドメインで配信する必要があり、GitHub Pagesではなくカスタムドメインに対応したホスティングへの移行が前提になります。
