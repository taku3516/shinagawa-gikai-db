...
会議録を足したり抜粋を作り直したりしたあとは、発言者名と議員IDの対応表を作り直します。

```bash
python3 scripts/build_speaker_members.py
```

`data/speaker-members.js`（32 KB）を書き出します。索引層には触れません。判定に使うのは次の4つです。

1. **任期は`data/elections.js`の区議選当選記録から作る**（当選した選挙の日〜次の一般選挙の日）。人物台帳の`seenIn`は「発言を確認できた年」で在職年ではないので使いません。
2. **会議の日付で判定する。** 区議選は4月下旬で年度IDは暦年（令和7年＝2025年、1月の委員会が`r07`に入る）なので、年単位だと選挙のあった年に前後で入れ替わった議員を取り違えます。
3. **括弧の中身は名の先頭1文字**（石田（ち）→ちひろ、原（雅）→雅美）。漢字・かなのどちらでも書かれます。
4. **別名の姓でも引く**（阿久津委員＝あくつ広王、小芝＝こしば）。議員本人の届出表記が任期の途中で変わることがあります。

これで161,816件のうち99.56%が自動で解けます。残りは同姓が同時に在職していて括弧が付いていないもので、`scripts/speaker-overrides.tsv`に手で書きます（現在149行）。

```
2007-11-07	石田委員	hideo_ishida
```

判定は品川区議会の会議録検索システム「発言者でさがす」で行いました。同姓の候補それぞれで発言者検索をして、その会議（日付＋会議名）が一覧に出るかを見ます。

**発言者検索には実在しない人物項目が混じることがあります。** 「鈴木雄二」は4文書を返しますが、区議選の当選・落選記録のどこにもおらず、その項目が指す2005年の発言は本文で「共産党の鈴木ひろ子です」と本人が名乗っています。候補の誰にも当たらないときは、索引そのものを疑って本文と委員会構成を確かめてください。

状況の確認は次のコマンドです。

```bash
python3 scripts/check_speaker_resolution.py           # 解決率と未解決の一覧
python3 scripts/check_speaker_resolution.py --strict  # 未解決があれば失敗（CIはこちら）
```

上書き表は議員IDの打ち間違いと、その日に在職していない人を指す行で失敗します。**この検査に例外を作らないでください。** 一度「区議選の当選記録が無い議員は在職を確かめられないので通す」という緩和を入れたところ、上記の誤りがそのまま通りました。
