...
本会議・委員会の質疑を**会議録の全文**で読めるようにするための設計です。
委員会は全文の掲載のみとし、抜き出した質疑は画面に出しません（抜き出した文は横断検索の
手がかりとして索引層に残します）。本会議は質問・答弁の要約と全文の両方を掲載します。GitHub Pagesのまま、既存のデータ形式・`file://`での直接起動・横断検索を壊さずに実現することを条件にしています。
