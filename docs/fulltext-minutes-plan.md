...
段階1・2（委員会の生成と表示）、段階3a（令和3〜7年の投入）、段階4（本会議）は実装済みです。実装した内容に合わせてこの文書も更新しています。残る21年（平成13年〜令和2年、令和8年）は会議録の取得を伴うため、`.github/workflows/rebuild-committees.yml` と `.github/workflows/rebuild-plenary-fulltext.yml` を年ごとに実行します。
...
|---|---:|---:|
| 日数 | 18日（定例会4回） | 約470日 |
| 全文の文字数 | 721,044字 | 約1,880万字 |
| UTF-8での容量 | 2.1 MB | **約55 MB** |
| 1日の平均 | 40,058字（約119 KB） | — |
...
質問者ごとに、その議員が質問を始めた発言の位置を記録します（採点は `prepare_history.question_start_score` と共通）。令和3〜7年では208名中207名の入口が取れました。本会議の要約は見出しの言い換えで終わっているものが34.9%あり（`check_qa_summaries.py`）、ここが委員会の `voiceIndex` 以上に効きます。
...
| **3a**（実装済み） | **令和3〜7年**の委員会を投入 | 724会議 / 約94 MB |
| **4**（実装済み） | 本会議の全文（`prepare_plenary_fulltext.py`）と令和3〜7年の投入 | 81日 / 約10 MB |
| **3b** | 委員会の残り21年を年ごとに投入（`rebuild-committees.yml`。1年1コミット） | 約2,576会議 / 約330 MB |
| **4b** | 本会議の残り21年（`rebuild-plenary-fulltext.yml`。1年1コミット） | 約380日 / 約45 MB |
| **5** | 全文検索（別設計） | — |

直近5年（令和3〜7年）で「委員会724会議・本会議81日の全文が読める」状態になっています。合計104 MBで、GitHub Pagesの1 GB上限に対してまだ十分な余裕があります。**残りを入れなくても、そこまでで価値が出る**構成にしてあります。
