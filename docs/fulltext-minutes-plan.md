...
段階1・2（委員会の生成と表示）、段階3a（令和6年の投入）、段階4（本会議）は実装済みです。実装した内容に合わせてこの文書も更新しています。残る段階3b（委員会の残り25年）と本会議の残り25年は会議録の取得を伴うため、`.github/workflows/rebuild-committees.yml` と `.github/workflows/rebuild-plenary-fulltext.yml` を年ごとに実行します。
...
質問者ごとに、その議員が質問を始めた発言の位置を記録します（採点は `prepare_history.question_start_score` と共通）。令和6年では40名全員の入口が取れました。本会議の要約は見出しの言い換えで終わっているものが34.9%あり（`check_qa_summaries.py`）、ここが委員会の `voiceIndex` 以上に効きます。
...
| **3a**（実装済み） | **令和6年**の委員会を投入 | 151会議 / 19.4 MB |
| **4**（実装済み） | 本会議の全文（`prepare_plenary_fulltext.py`）と令和6年の投入 | 18日 / 2.1 MB |
| **3b** | 委員会の残り25年を年ごとに投入（`rebuild-committees.yml`。1年1コミット） | 3,149会議 / 約401 MB |
| **4b** | 本会議の残り25年（`rebuild-plenary-fulltext.yml`。1年1コミット） | 約450日 / 約53 MB |
| **5** | 全文検索（別設計） | — |

令和6年だけで「委員会151会議・本会議18日の全文が読める」状態になっています。表示の重さ・読み込み時間・法務面を実データで確認してから、残りを入れる判断ができます。**残りを入れなくても、そこまでで価値が出る**構成にしてあります。

### 令和6年で確かめたこと

- 委員会1会議あたり平均131 KB（見込み132 KB）、本会議1日あたり平均119 KB。開いたときだけ読む方式で問題ありません
- 抜粋7,584件すべてに `voiceIndex` が付き、全文の同じ発言者を指しています
- 本会議の質問者40名すべてに入口が付き、その議員が質問を始めた発言を指しています
